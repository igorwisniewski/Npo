...
export default function CounterCard({ title,description, icon: Icon, count, onIncrement, onDecrement }: Props) {
    ...
    return (
      ...
      ${isActive ? 'border-red-500 bg-red-500/10' : 'border-red-700 bg-brand-bg-light'}`}>

            {/* Kontener na tekst i ikonę */}
            <div className="flex items-center w-full">
                <Icon className={`h-8 w-8 mr-4 flex-shrink-0 transition-colors ${isActive ? 'text-red-500' : 'text-red-800'}`} />
            ...
            <div className="flex items-center gap-3 mt-4 sm:mt-0 self-end sm:self-center">
                ...
                {isActive && (
                    <button
                        type="button"
                        onClick={onDecrement}
                        className="w-10 h-10 rounded-full bg-red-700 text-white flex items-center justify-center hover:bg-red-300 transition-colors"
                    >
                        <FaMinus />
                    </button>
                )}
