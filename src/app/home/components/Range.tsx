...
export default function RangeSlider({ label, min, max, step, value, onChange, name, formatLabel }: Props) {
    return (
        <div className="w-full">
            ...
            <span className="px-4 py-1 text-white bg-red-800 rounded-full font-bold text-lg">
                {formatLabel(value)}
            </span>
                </div>
                <input
                    ...
                    name={name}
                    onChange={onChange}
                    className="w-full h-2 bg-red-700 rounded-lg appearance-none cursor-pointer accent-red-800"
                />
