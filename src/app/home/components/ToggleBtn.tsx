    ...
    return (
        <div className={`w-full mx-auto grid ${gridLayoutClass} gap-4`}>
            {options.map((option) => {
              ...
              ${isSelected
                            ? 'bg-red-800 border-red-800 shadow-lg shadow-red-500/20'
                            // Używamy `brand-bg-light` i `brand-border` z Twojego pliku tailwind.config.ts
                            : 'bg-brand-bg-light border-red-800    border hover:border-red-500'
                        }`}
                    >
                        {option.icon && <option.icon className={`mb-2 h-7 w-7 ${isSelected ? 'text-white' : 'text-red-800'}`} />}
